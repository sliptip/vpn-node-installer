...
После установки узла можно использовать отдельный `clients.sh`. Файл со списком на сервер загружать не нужно: список вставляется прямо в SSH-терминал.
...
После нормализации клиент получает имя вида `alice@vpn.orion.example`, то есть исходный почтовый домен не переносится на VPN-узел. Пустые строки и Markdown-разделители игнорируются, `\@` понимается как обычный `@`, повторы после нормализации удаляются.

Перед изменениями показывается предпросмотр и запрашивается подтверждение. Уже существующие `name@<домен-узла>` не перезаписываются. Перед массовым созданием делается root-only SQLite backup в `/etc/vpn-node-installer/backups/`. UUID v4 генерируется отдельно для каждого нового клиента, после записи каждый созданный клиент перепроверяется через API.
...
`.github/workflows/validate.yml` проверяет `install.sh`. Отдельный `.github/workflows/validate-clients.yml` проверяет Bash-синтаксис и ShellCheck для `clients.sh`, а также запускает встроенный self-test нормализации входного списка.
